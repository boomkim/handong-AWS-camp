...
- 1.1.9 VPC가 생성되면, VPC가 성공적으로 생성되었다는 메시지가 표시된 페이지가 보입니다. `[OK]`를 클릭합니다.
...
- 1.2.9 `[Private_Subnet2]`를 선택하고, 모든 다른 서브넷이 선택 해제되었는지 확인한 다음
    ...
    - `[Destination] 0.0.0.0/0`의 `[Target]`이 접두사 eni를 포함하도록 변경합니다. `[Save]`를 클릭합니다.
...
- 2.1.5 `[Step3 : Configure Instance Details]` 페이지에서 다음 정보를 입력하고 나머지 값은 모두 기본값으로 둡니다.
    ...
    - `[Auto-assign PublicIP]` : Enable을 클릭
...
```
- 2.1.8 `[Next]` : Add Storage를 클릭합니다. 
- 2.1.9 `[Next]` : Add Tags를 클릭합니다. 
...
- 1.2.5	`[Create Subnet]` 대화 상자에서 다음 세부 정보를 입력합니다.
    ...
    - `[VPC]` : My Lab VPC 선택
    - `[Availability Zone]` : 앞에서 퍼블릿 서브넷1용으로 적어둔 가용 영역을 클릭
