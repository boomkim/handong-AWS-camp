...
AWS VPC를 생성하면 각 가용 영역에 하나 이상의 서브넷(Subnet)을 추가할 수 있습니다. 각 서브넷은 하나의 가용 영역 내에 모두 상주하며, 다른 영역으로 확장할 수 없습니다. 서브넷 트래픽이 인터넷게이트웨이(IGW)로 라우팅되는 경우 해당 서브넷을 퍼블릭 서브넷(Public Subnet)이라고 합니다. 서브넷이 인터넷게이트웨이(IGW)로 라우팅되지 않으면 해당 서브넷을 프라이빗 서브넷(Private Subnet)이라고 합니다.
...
- 1.3.7 `[규칙 편집]`을 클릭
- 1.3.8 `[규칙 추가]`를 클릭
- 1.3.9 `[유형]`에서 스크롤을 내려 **HTTP(80)**를 클릭
- 1.3.10 `[소스]` 입력란에 **0.0.0.0/0**를 입력
- 1.3.11 `[규칙 추가]`를 클릭
- 1.3.12 `[유형]`에서 스크롤을 내려 **SSH(22)**를 클릭
- 1.3.13 `[소스]` 입력란에  **0.0.0.0/0**을 입력
- 1.3.14 `[저장]`를 클릭 후 `[닫기]` 클릭
...
본 작업에서는 VPC에서 EC2 인스턴스를 시작하는 과정을 확인. 이 인스턴스는 웹 서버의 역할을 합니다.
